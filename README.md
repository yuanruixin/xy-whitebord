...
- **AI 生成图形**：右侧非模态对话侧边栏，配置 API Key（兼容 OpenAI 接口）后，用自然语言多轮对话；需求明确直接生成图形与连线并导入画布，需求含糊会先追问澄清
...
2. 在底部输入框描述需求（`Enter` 发送，`Shift+Enter` 换行）。模型会先判断需求是否明确：
   - 明确：直接生成并绘制到画布；
   - 含糊：先提出 1~3 个澄清问题，回答后继续，直到明确再绘制。
   - 生成时会带上当前画布已有元素与可见区域信息，新图形会避开已有内容、避免重叠。
3. 生成过程为流式输出，助手消息会展示思考内容与当前阶段（连接 / 思考 / 生成 / 解析）。
4. 生成完成后图形自动绘制到画布，可继续编辑、撤销；生成中可点击停止；历史中的场景消息可「重新导入画布」。
