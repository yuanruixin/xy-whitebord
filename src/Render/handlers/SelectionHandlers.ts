...
export class SelectionHandlers implements Types.Handler {
        ...
        if (
          ...
          this.render.selectRect.setAttrs({
            ...
            x: this.render.toStageValue(
              Math.min(this.selectRectStartX, this.selectRectEndX) -
                stageState.x
            ),
            y: this.render.toStageValue(
              Math.min(this.selectRectStartY, this.selectRectEndY) -
                stageState.y
            ),
            width: this.render.toStageValue(
              Math.abs(this.selectRectEndX - this.selectRectStartX)
            ),
            height: this.render.toStageValue(
              Math.abs(this.selectRectEndY - this.selectRectStartY)
            ),
          });
        }
      },
  ...
  attract = (newPos: Konva.Vector2d) => {
    ...
    if (this.render.config.attractNode) {
      ...
      // 拖动目标
      const targetIds = this.render.selectionTool.selectingNodes.map(
        (o) => o._id
      );
      // 除拖动目标的其他
      const otherNodes = this.render.layer.getChildren(
        (node) => !targetIds.includes(node._id)
      );
      ...
      if (pairXMin[0]) {
        if (
          Math.abs(pairXMin[0][0].value - pairXMin[0][1].value) <
          this.render.bgSize / 2
        ) {
          pairX = pairXMin[0];
        }
      }
      ...
      if (pairYMin[0]) {
        if (
          Math.abs(pairYMin[0][0].value - pairYMin[0][1].value) <
          this.render.bgSize / 2
        ) {
          pairY = pairYMin[0];
        }
      ...
      if (pairX?.length === 2) {
              ...
              points: [
                ...
                [
                  ...
                  this.render.toStageValue(
                    this.render.stage.height() - stageState.y
                  ),
                ],
              ].flat(),
              stroke: "blue",
        ...
        if (target && other) {
          // 磁铁坐标值
          newPosX =
            newPosX - this.render.toBoardValue(target.value - other.value);
          isAttract = true;
        }
      }
      ...
      if (pairY?.length === 2) {
          ...
          if (other) {
            ...
            const line = new Konva.Line({
              points: [
                ...
                [
                  this.render.toStageValue(
                    this.render.stage.width() - stageState.x
                  ),
                  other.value,
                ],
              ].flat(),
        ...
        if (target && other) {
          // 磁铁坐标值
          newPosY =
            newPosY - this.render.toBoardValue(target.value - other.value);
          isAttract = true;
        }
      }
    }
    ...
    if (this.render.config.attractBg) {
      ...
      if (pairX === null) {
        ...
        const logicRightX = this.render.toStageValue(
          newPos.x + width - stageState.x
        ); // x坐标
        ...
        for (const diff of [
          ...
          if (diff.value < 5) {
            if (diff.type === "stageRightX") {
              newPosX =
                this.render.toBoardValue(logicStageRightX) +
                stageState.x -
                width;
            } else if (diff.type === "leftX") {
              newPosX =
                this.render.toBoardValue(logicClosestLeftX) + stageState.x;
            } else if (diff.type === "rightX") {
              newPosX =
                this.render.toBoardValue(logicClosestRightX) +
                stageState.x -
                width;
            }
            isAttract = true;
      ...
      if (pairY === null) {
        ...
        const logicBottomY = this.render.toStageValue(
          newPos.y + height - stageState.y
        ); // y坐标
        ...
        const logicDiffStageBottomY = Math.abs(
          logicBottomY - logicStageBottomY
        );
        ...
        for (const diff of [
          ...
          if (diff.value < 5) {
            if (diff.type === "stageBottomY") {
              newPosY =
                this.render.toBoardValue(logicStageBottomY) +
                stageState.y -
                height;
            } else if (diff.type === "topY") {
              newPosY =
                this.render.toBoardValue(logicClosestTopY) + stageState.y;
            } else if (diff.type === "bottomY") {
              newPosY =
                this.render.toBoardValue(logicClosestBottomY) +
                stageState.y -
                height;
            }
            isAttract = true;
